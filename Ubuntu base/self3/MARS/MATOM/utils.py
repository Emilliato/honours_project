...
def get_path(method=""):
    ...
    if home.split('/')[0] == '':
        ...
        static_file = '%s/code/MARS/static/files/%s' % (home, method)
        meme_path = "/opt/conda/envs/MARS/bin"
...
def handle_uploaded_bed(tf, length, uploaded_chipseq, results_folder):
    ...
    if bed_is_correct[0]:
        os.system("%s/MARSTools/bed2chipseg.sh %s/%s %s/%s/%s_tmp_chip_%s %s/Data/hg19.fa" %
